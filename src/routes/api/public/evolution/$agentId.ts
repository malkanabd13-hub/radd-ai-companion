      ...
      POST: async ({ request, params }) => {
        ...
        const system = [
          ...
          agent.employee_number
            ? `إذا طلب العميل صراحةً التحدث مع موظف بشري، أو كان طلبه خارج البيانات المتاحة تمامًا، لا تعطه أي رقم تواصل إطلاقًا، بل أخبره أنه سيتم تحويله لموظف، وأضف في آخر ردّك سطرًا منفصلًا بهذا الشكل حرفيًا:\n[HANDOFF] <ملخص قصير لطلب العميل وما دار في المحادثة>\nهذا السطر داخلي ولن يراه العميل. ممنوع منعًا باتًا ذكر أي أرقام هواتف داخلية أو أرقام موظفين.`
          ...
          dbKnowledge
            ? [
                `لديك أدناه بيانات فعلية ومحدّثة من قاعدة بيانات العميل، كل سطر يمثل سجلًا كاملًا بحقوله.`,
                `قواعد إلزامية:`,
                `- ابحث داخل هذه السجلات بنفسك قبل أي رد، وطابق طلب العميل (الموقع، السعر، عدد الغرف، النوع...) مع قيم الحقول.`,
                `- ممنوع أن تقول إنك لا تملك التفاصيل أو لا تستطيع البحث، فالسجلات أمامك.`,
                `- اذكر النتائج المطابقة بتفاصيلها (الاسم/الموقع/السعر/الغرف وأي حقول مهمة)، وإن كانت كثيرة اعرض أفضل 3 إلى 5 واسأل العميل إن أراد المزيد.`,
                `- إذا لم يوجد أي سجل مطابق، قل ذلك بوضوح واقترح أقرب البدائل الموجودة في السجلات.`,
                `- لا تحوّل العميل لموظف بسبب البحث في البيانات، التحويل فقط عند طلبه موظفًا أو عند أمر خارج نطاق البيانات.`,
                `البيانات:`,
                dbKnowledge,
              ].join("\n")
            : "",
